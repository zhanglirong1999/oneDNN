...
void mlp_fuse_example(dnnl::engine::kind engine_kind) {
    ...
    memory::dims weights1_dims = {MB, K, HIDDEN_SIZE}; // M*K 与K*HIDDEN_SIZE
    ...
    // Create memory descriptors and memory objects for src, weights1, bias1, and
    // hidden.
    ...
    auto matmul_pd = matmul::primitive_desc(
            engine, src_md, weights1_md, bias1_md, hidden_md, matmul_attr);
    ...
    // second layer mutmul + ReLU
    const float alpha2 = 0.7;
    const float beta2 = 0.f;

    post_ops matmul_ops2;
    matmul_ops2.append_eltwise(algorithm::eltwise_relu, alpha2, beta2);
    primitive_attr matmul_attr2;
    matmul_attr2.set_post_ops(matmul_ops2);

    auto matmul_pd2 = matmul::primitive_desc(
            engine, hidden_md, weights2_md, bias2_md, dst_md, matmul_attr2);
    ...
    std::cout << "Elapsed time on double mlp with post-ops fuse ReLU: " << elapsed_time << " ms" << std::endl;

}

int main(int argc, char **argv) {
    // handle_example_errors(mlp_example, parse_engine_kind(argc, argv));
    return handle_example_errors(mlp_fuse_example, parse_engine_kind(argc, argv));
}
